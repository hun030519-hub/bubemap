...
파이썬 스크립트나 API 크롤러를 통해 부산 공공데이터포털(data.busan.go.kr)의 데이터를 이 테이블에 주기적으로 동기화합니다.

Project Settings > API에서 URL과 anon public API key를 확인합니다.

로컬 실행
config.example.js를 복사해 config.local.js를 만들고 Supabase 값을 넣습니다. config.local.js는 Git에 올라가지 않습니다.
...
배포가 완료되면 대시보드에 생성된 퍼블릭 URL을 통해 사이트에 접속합니다.
